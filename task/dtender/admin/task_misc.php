		...
		if ($ac && $work_id) {
			switch ($ac) {
				case 'del' : 
					...
					$res = db_factory::execute ( sprintf ( 'delete  from %switkey_task_bid where bid_id=%d', TABLEPRE, intval ( $work_id ) ) );
					if ($res) {
				...
				case 'comm' : 
					$c_list = db_factory::query ( sprintf ( ' select a.content,a.on_time from %switkey_comment a 
						left join %switkey_task_work b on a.obj_id=b.bid_id where b.bid_id=%d', TABLEPRE, TABLEPRE, $work_id ) );
					break;
			}
