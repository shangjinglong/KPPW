...
SWFUpload.Console.writeLine = function(g) {
	...
	try {
		...
		tipsOp("Exception: " + h.name + " Message: " + h.message);
						...
						if (e[0]) {
							if (e[0] == "*") {
								...
								for ( var m = 0; m < e.length; m++) {
									i.cancelUpload(e[m]);
									c("#" + e[m]).find(".data").removeClass(
											"data").html(" - Cancelled");
									c("#" + e[m]).find(
											".uploadify-progress-bar").remove();
									c("#" + e[m]).delay(1000 + 100 * m)
...
//					tipsOp(this.queueData.errorMsg);
				}
			}
			if (e.onDialogClose) {
...
//						this.queueData.errorMsg += "\n选定文件数超过上传限制 ("+ f + ").";
		...
		onUploadError : function(e, i, h) {
			...
			case SWFUpload.UPLOAD_ERROR.UPLOAD_LIMIT_EXCEEDED:
				tipsOp('上传文件数量超出限制');
				//tipsOp("The upload limit has been reached (" + h + ").");
